...
import {
  X, ArrowLeft, ArrowRight, Sparkles, Heart, ShoppingBag,
  RefreshCw, Share2, Bookmark, Check, Star, Pencil, ImageOff,
} from "lucide-react";
...
const OCCASIONS = [
  "عيد ميلاد", "ذكرى سنوية", "زفاف", "تخرج", "عيد الحب",
  "عيد الأم", "عيد الأب", "تهنئة", "سلامات", "شكر", "بدون مناسبة",
];

const FLOWERS = ["ورد جوري", "توليب", "زنبق", "فاوانيا", "أوركيد", "عباد الشمس", "هيدرانجيا", "قرنفل"];
...
const initial: Answers = {
  recipient: "", occasion: "", budget: 495,
  flowers: [], colors: [], style: "", delivery: "",
};
...
async function fetchPerenualData(flowerName: string): Promise<Partial<FlowerData> | null> {
  ...
  try {
    ...
    return {
      ...
      ].filter(Boolean).join(" · ") || "يُنصح بالري المعتدل وضوء غير مباشر",
...
// ─────────────────────────────────────────────────────────────────────────────
async function getAIRecommendations(answers: Answers, shopProducts: typeof products): Promise<AIRec[]> {
  ...
  try {
    const anthropicRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: "أنت مساعد متجر زهور. أجب دائماً بـ JSON فقط بدون أي نص إضافي أو markdown backticks.",
      ...
      try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            temperature: 0.3,
            messages: [
              { role: "system", content: "أنت مساعد متجر زهور. أجب دائماً بـ JSON فقط بدون أي نص إضافي أو backticks." },
        ...
        return gparsed.recommendations.map((r: any) => {
          ...
          return {
            ...
            match: r.match ?? 95, reason: r.reason ?? "توصية مخصصة",
          };
        });
      } catch { /* fall to local scoring */ }
    }
    ...
    return shopProducts
      .map(p => {
        let score = 60;
        const searchStr = `${p.name} ${p.tagline} ${p.meaning} ${p.occasion?.join(" ")}`.toLowerCase();
...
const FLOWER_LOCAL: Record<string, { meaning: string; care: string; occasions: string[] }> = {
  "ورد جوري":   { meaning: "رمز الحب والجمال والعاطفة الصادقة", care: "ري كل يومين · ضوء غير مباشر · قطع السيقان قطراً", occasions: ["عيد الحب", "ذكرى سنوية", "زفاف"] },
  "توليب":      { meaning: "إعلان الحب المثالي والكمال", care: "ري معتدل · بارد · بعيد عن الشمس المباشرة", occasions: ["عيد الحب", "عيد الأم", "تهنئة"] },
  "زنبق":       { meaning: "النقاء والتجديد والأناقة", care: "تربة رطبة · ضوء كافٍ · إزالة المتبرة", occasions: ["زفاف", "ذكرى سنوية", "عيد الأم"] },
  "فاوانيا":    { meaning: "الازدهار والحظ والثراء", care: "ري منتظم · ضوء جزئي · لا تزحم في الإناء", occasions: ["زفاف", "ذكرى سنوية", "تهنئة"] },
  "أوركيد":     { meaning: "الفخامة والغموض والجمال النادر", care: "ري مرة أسبوعياً · ضوء غير مباشر · لا تتراكم المياه", occasions: ["ذكرى سنوية", "تهنئة", "بدون مناسبة"] },
  "عباد الشمس": { meaning: "البهجة والتفاؤل والإخلاص", care: "ري يومي · ضوء شمس كامل · تربة خصبة", occasions: ["تهنئة", "تخرج", "عيد ميلاد"] },
  "هيدرانجيا":  { meaning: "الامتنان والوفرة وعمق المشاعر", care: "ري وفير · ظل جزئي · رطوبة مرتفعة", occasions: ["زفاف", "عيد الأم", "شكر"] },
  "قرنفل":      { meaning: "الحب العميق والاحترام والإعجاب", care: "ري خفيف · ضوء معتدل · هواء نظيف", occasions: ["عيد الأم", "شكر", "تهنئة"] },
};
...
async function enrichWithFlowerData(flowers: string[]): Promise<FlowerData[]> {
  ...
  const [img, info] = await Promise.all([
    fetchUnsplashImage(enName),
    fetchPerenualData(enName),
  ]);
...
export function AIBouquetWizard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const canNext = useMemo(() => {
    switch (step) {
      case 0: return !!answers.recipient;
      case 1: return !!answers.occasion;
      case 2: return answers.budget > 0;
      case 3: return answers.flowers.length > 0;
      case 4: return answers.colors.length > 0;
      case 5: return !!answers.style;
      case 6: return !!answers.delivery;
      default: return false;
    }
  }, [step, answers]);

  const reset = () => { setAnswers(initial); setStep(0); setPhase("wizard"); setRecs([]); setFlowerData([]); };
  ...
  const generate = async () => {
    ...
    try {
      ...
      setRecs(products.slice(0, 4).map((p, i) => ({
        slug: p.slug, name: p.name, tagline: p.tagline,
        price: p.price, currency: p.currency, image: p.image,
        rating: p.rating, reviews: p.reviews, match: 95 - i * 4,
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-charcoal/40 backdrop-blur-md animate-reveal" dir="rtl">
      ...
      <div className="relative flex-1 flex flex-col bg-cream m-2 sm:m-6 rounded-[28px] overflow-hidden shadow-luxe">
        ...
        <div className="flex-1 overflow-y-auto">
          {phase === "wizard"  && (
            <WizardBody
              step={step} setStep={setStep} answers={answers}
              setAnswers={setAnswers} canNext={canNext} onGenerate={generate}
            />
          )}
          {phase === "loading" && <LoadingBody msg={loadMsg} />}
          {phase === "results" && (
            <ResultsBody
              recs={recs} flowerData={flowerData}
...
// ─────────────────────────────────────────────────────────────────────────────
function WizardBody({
  step, setStep, answers, setAnswers, canNext, onGenerate,
}: {
  step: number; setStep: (n: number) => void; answers: Answers;
  setAnswers: (a: Answers) => void; canNext: boolean; onGenerate: () => void;
}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 sm:px-10 py-10">
      ...
      <div className="mt-8 text-center">
        <div className="eyebrow mb-3">الخطوة {step + 1} من {STEP_TITLES.length}</div>
      ...
      <div key={step} className="mt-10 animate-reveal">
        {step === 0 && (
          <Grid cols="grid-cols-2 md:grid-cols-4">
            {RECIPIENTS.map((r) => (
              <OptionCard key={r.key} active={answers.recipient === r.label} onClick={() => setAnswers({ ...answers, recipient: r.label })}>
        ...
        {step === 1 && (
          <div className="flex flex-wrap justify-center gap-3">
            {OCCASIONS.map((o) => (
              <Chip key={o} active={answers.occasion === o} onClick={() => setAnswers({ ...answers, occasion: o })}>{o}</Chip>
            ))}
          </div>
        )}
        {step === 2 && (
          <div className="max-w-lg mx-auto text-center">
            <div className="font-display text-5xl">{answers.budget} <span className="text-lg text-muted-foreground">ج.م</span></div>
            <input
              type="range" min={20} max={3000} step={10} value={answers.budget}
              onChange={(e) => setAnswers({ ...answers, budget: Number(e.target.value) })}
              className="w-full mt-8 accent-rose-gold" aria-label="الميزانية"
            />
            <div className="flex justify-between text-xs text-muted-foreground mt-2">
              <span>20 ج.م</span><span>3000 ج.م</span>
            </div>
          </div>
        )}
        {step === 3 && (
          <div className="flex flex-wrap justify-center gap-3">
            {FLOWERS.map((f) => (
              <Chip key={f} active={answers.flowers.includes(f)} onClick={() => setAnswers({
                ...answers,
                flowers: answers.flowers.includes(f) ? answers.flowers.filter(x => x !== f) : [...answers.flowers, f],
              })}>{f}</Chip>
            ))}
          </div>
        )}
        {step === 4 && (
          <div className="flex flex-wrap justify-center gap-5">
            {COLORS.map((c) => {
              ...
              return (
                <button key={c.key} onClick={() => setAnswers({
                  ...answers,
                  colors: active ? answers.colors.filter(x => x !== c.label) : [...answers.colors, c.label],
                })} className="flex flex-col items-center gap-2 group" aria-pressed={active}>
                  <span className={`w-16 h-16 rounded-full border-2 transition-all shadow-soft ${active ? "border-rose-gold scale-110" : "border-transparent group-hover:scale-105"}`}
        ...
        {step === 5 && (
          <Grid cols="grid-cols-2 md:grid-cols-4">
            {STYLES.map((s) => (
              <OptionCard key={s} active={answers.style === s} onClick={() => setAnswers({ ...answers, style: s })}>
        ...
        {step === 6 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
            {DELIVERY.map((d) => (
              <OptionCard key={d} active={answers.delivery === d} onClick={() => setAnswers({ ...answers, delivery: d })}>
      ...
      <div className="mt-12 flex items-center justify-between gap-4">
        ...
        {isLast ? (
          <button onClick={onGenerate} disabled={!canNext} className="btn-luxe disabled:opacity-40 disabled:pointer-events-none">
            <Sparkles className="w-4 h-4" /> أنشئ توصياتي بالذكاء الاصطناعي
          </button>
        ) : (
          <button onClick={() => setStep(step + 1)} disabled={!canNext} className="btn-luxe disabled:opacity-40 disabled:pointer-events-none">
  ...
  return (
    <div className="flex items-center gap-2">
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-500 ${i <= step ? "bg-rose-gold" : "bg-border"}`} />
      ))}
    </div>
  );
}
  ...
  return (
    <button onClick={onClick} aria-pressed={active}
      className={`px-5 py-2.5 rounded-full text-sm transition-all border ${
        active ? "bg-charcoal text-primary-foreground border-charcoal" : "bg-background border-border hover:border-rose-gold"
      }`}>
      {children}
    </button>
  );
    ...
    <div className="min-h-full grid place-items-center py-24 px-6">
      <div className="text-center max-w-md">
        <div className="relative w-32 h-32 mx-auto mb-10">
          ...
          <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-rose-gold animate-spin" style={{ animationDuration: "1.4s" }} />
        ...
        <div key={msg} className="font-display text-2xl animate-reveal">{msg}</div>
        <p className="text-xs text-muted-foreground mt-4">الذكاء الاصطناعي يحلل تفضيلاتك ويجلب بيانات الزهور...</p>
      </div>
    </div>
  );
}
...
function ResultsBody({
  recs, flowerData, onEdit, onRegen, onReset,
}: { recs: AIRec[]; flowerData: FlowerData[]; onEdit: () => void; onRegen: () => void; onReset: () => void }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 sm:px-10 py-10">
      <div className="text-center mb-10">
        ...
        <p className="text-muted-foreground mt-3">توصيات مُولَّدة بالذكاء الاصطناعي — مبنية على أسلوبك ومناسبتك وميزانيتك</p>
      </div>
      ...
      {primaryFlower && (
        <div className="mb-8 p-5 rounded-2xl border border-rose-gold/20 bg-blush/20 grid md:grid-cols-[auto_1fr] gap-5 items-center">
          <div className="w-20 h-20 rounded-xl overflow-hidden bg-cream flex-shrink-0">
            {primaryFlower.image ? (
              <img src={primaryFlower.image} alt={primaryFlower.arabicName} className="w-full h-full object-cover" />
            ) : (
          ...
          <div>
            <div className="eyebrow text-rose-gold mb-1">
              معلومات الزهرة · {primaryFlower.source === "unsplash" ? "صورة Unsplash" : primaryFlower.source === "perenual" ? "بيانات Perenual" : "مصدر محلي"}
            </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
        {recs.map((r) => (
          <article key={r.slug} className="group relative rounded-3xl overflow-hidden bg-background border border-border/60 hover:shadow-luxe transition-all">
            <div className="grid grid-cols-[1.1fr_1fr]">
              <div className="relative aspect-square overflow-hidden bg-cream">
                <img src={r.image} alt={r.name} className="w-full h-full object-cover transition-transform duration-[1200ms] group-hover:scale-105" />
              ...
              <div className="p-5 flex flex-col">
                ...
                <div className="flex items-center gap-1.5 mt-2 text-xs text-muted-foreground">
                  <Star className="w-3.5 h-3.5 fill-rose-gold text-rose-gold" /> {r.rating} · {r.reviews}
                </div>
                <div className="font-display text-lg mt-3">{r.price} <span className="text-xs text-muted-foreground">{r.currency}</span></div>
                <p className="text-xs text-muted-foreground mt-3 leading-relaxed border-t border-border/60 pt-3">
                  <span className="text-rose-gold">YasRose AI: </span>{r.reason}
                </p>
                <div className="mt-auto pt-4 grid grid-cols-2 gap-2">
                  <button className="btn-luxe !py-2 !text-xs">
                    <ShoppingBag className="w-3.5 h-3.5" /> أضف للسلة
                  </button>
                  <button className="rounded-full py-2 text-xs border border-border hover:border-rose-gold transition">تفاصيل</button>
                  ...
                  <button className="rounded-full py-2 text-xs border border-border hover:border-rose-gold transition">قارن</button>
                </div>
              </div>
            </div>
          </article>
      ...
      <div className="mt-12 flex flex-wrap items-center justify-center gap-3">
        <button onClick={onEdit}  className="btn-ghost-luxe"><Pencil    className="w-4 h-4" /> عدّل إجاباتك</button>
        <button onClick={onRegen} className="btn-luxe">      <RefreshCw className="w-4 h-4" /> أنشئ توصيات جديدة</button>
        <button onClick={onReset} className="btn-ghost-luxe"><Sparkles  className="w-4 h-4" /> ابدأ من جديد</button>
        <button className="btn-ghost-luxe"><Bookmark className="w-4 h-4" /> احفظ</button>
        <button className="btn-ghost-luxe"><Share2   className="w-4 h-4" /> شارك</button>
      </div>
    </div>
  );
}
